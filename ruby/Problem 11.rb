...
big_x = 0
big_y = 0
puts grid[6][8] * grid[6+1][8+1] * grid[6+2][8+2] * grid[6+3][8+3]
...
(0..16).each do |x|
  (0..16).each do |y|
    ...
    # product diagonally up-right
    tmp = grid[x+3][y] * grid[x+2][y+1] * grid[x+1][y+2] * grid[x][y+3]
    if tmp > biggest
      biggest = tmp 
      puts "#{biggest} @ #{x}, #{y} UR"
      big_x = x
